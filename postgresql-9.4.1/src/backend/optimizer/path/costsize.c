...
/* added by rajmohan for quest selectivity injection */
char*       injection_string = NULL;
			 ...
			 RelOptInfo *baserel, ParamPathInfo *param_info)
{
	...
	path->cost2 = seq_page_cost;
	path->startup_tuple = baserel->baserestrictcost.startup;
	path->per_tuple = cpu_per_tuple;
}
...
//	QualCost 	index_qual_cost;
	...
	path->index_correlation = indexCorrelation;					/* <Added on 12/05/2015 for QUEST> */
	...
	path->path.cost1 = random_page_cost;				//random page cost.
	path->path.cost2 = seq_page_cost;					//Seq page cost
	path->path.startup_tuple = qpqual_cost.startup;
	path->path.per_tuple = cpu_per_tuple;

	path->loop_count = loop_count;
	path->total_table_pages = root->total_table_pages;
	path->index_correlation=csquared;

	//per page cost for index page fetch.
	path->spc_random_page_cost=random_page_cost;

//	cost_qual_eval(&index_qual_cost, indexQuals, root);
	qual_arg_cost = qpqual_cost.startup + qpqual_cost.per_tuple;
	qual_op_cost = cpu_operator_cost * (list_length(list_difference_ptr(allclauses, path->indexquals)));

	qual_arg_cost -= qual_op_cost;
	if (qual_arg_cost < 0)		/* just in case... */
		qual_arg_cost = 0;

	path->qual_arg_cost=qual_arg_cost;
	path->per_tuple_cost=cpu_index_tuple_cost+qual_op_cost;

	ListCell   *l;
	foreach(l, list_difference_ptr(allclauses, path->indexquals))
	{
	...
	path->num_sa_scans=num_sa_scans;
					  ...
					  Path *bitmapqual, double loop_count)
{
	...
	/* <Added on 12/05/2015 for QUEST> */
	path->cost1 = get_indexpath_pages(bitmapqual);
	path->cost2 = root->total_table_pages;
	path->startup_tuple = baserel->baserestrictcost.startup;
	path->per_tuple = cpu_per_tuple;
}
			 ...
			 RelOptInfo *baserel, List *tidquals, ParamPathInfo *param_info)
{
	...
	/* <Added on 12/05/2015 for QUEST> */
	path->cost2 = random_page_cost;
	path->startup_tuple = startup_cost;
	path->per_tuple = cpu_per_tuple;
}
		  ...
		  double limit_tuples)
{
	...
	/* <Added on 12/05/2015 for QUEST> */
	path->cost2 = (seq_page_cost * 0.75 + random_page_cost * 0.25);
	path->startup_tuple = 2.0 * cpu_operator_cost;					//comparison_cost
	path->per_tuple = cpu_operator_cost;
}
...
{
#ifndef MATERIAL_FOR_INNERPATH		/* <Added on 12/05/2015 for QUEST> */
	...
	path->startup_tuple = 0;
	path->per_tuple = cpu_tuple_cost * 1.1;				//cpu_tuple_cost * 0.1 + cpu_tuple_cost
	...
	path->startup_tuple = 0;
	path->per_tuple = 2*cpu_operator_cost;
		 ...
		 double input_tuples)
{
	...
	if (aggstrategy == AGG_PLAIN)
	{
		...
		/* <Added on 12/05/2015 for QUEST> */
//		path->startup_tuple = aggcosts->transCost.startup+aggcosts->finalCost;
		path->per_tuple = aggcosts->transCost.per_tuple ;
		path->cost2 = cpu_tuple_cost;
	}
	else if (aggstrategy == AGG_SORTED)
	{
		...
		/* <Added on 12/05/2015 for QUEST> */
		path->per_tuple = cpu_operator_cost * (numGroupCols);
		path->cost2 = cpu_operator_cost + cpu_tuple_cost;	/* This variable is used in for costing of per output tuple. */
	}
	else
	{
		/* must be AGG_HASHED */
		...
		/* <Added on 12/05/2015 for QUEST> */
		path->per_tuple = cpu_operator_cost * (numGroupCols);
		path->cost2 = cpu_operator_cost + cpu_tuple_cost;
	}
					...
					SemiAntiJoinFactors *semifactors)
{
	...
	/* <Added on 12/05/2015 for QUEST> */
	path->path.startup_tuple = restrict_qual_cost.startup;
	path->path.per_tuple = cpu_per_tuple;
					 ...
					 SpecialJoinInfo *sjinfo)
{
	...
	/* <Added on 12/05/2015 for QUEST> */
	path->jpath.path.cost1 = merge_qual_cost.startup;
	path->jpath.path.cost2 = merge_qual_cost.per_tuple;
	path->jpath.path.startup_tuple = qp_qual_cost.startup;
	path->jpath.path.per_tuple = cpu_per_tuple;
					...
					SemiAntiJoinFactors *semifactors)
{
	...
	/* <Added on 12/05/2015 for QUEST> */
	path->jpath.path.cost1 = hash_qual_cost.startup;
	path->jpath.path.cost2 = hash_qual_cost.per_tuple;
	path->jpath.path.startup_tuple = qp_qual_cost.startup;;
	path->jpath.path.per_tuple = cpu_per_tuple;
}
