...
/* Modified on 12/05/2015 for QUEST */
static TupleTableSlot *agg_retrieve_direct(AggState *aggstate, double *tuples);
 ...
 */
TupleTableSlot *
ExecAgg(AggState *node, double *tuples)		/* <Modified arguments on 12/05/2015 for QUEST> */
{
	/*
	...
	if (((Agg *) node->ss.ps.plan)->aggstrategy == AGG_HASHED)
	{
		...
		return agg_retrieve_direct(node,tuples); /* Modified on 12/05/2015 for QUEST */
}
...
agg_retrieve_direct(AggState *aggstate, double *tuples) // added by rajmohan for quest
{
	Agg		   *node = (Agg *) aggstate->ss.ps.plan;
	PlanState  *outerPlan;
	...
	while (!aggstate->agg_done)
	{
		...
		if (aggstate->grp_firstTuple == NULL)
		{
			...
			if (!TupIsNull(outerslot))
			{
				...
				(*tuples)++;				/* <Added on 12/05/2015 for QUEST> */
			}
			else
			{
				/* <Added on 12/05/2015 for QUEST> */
