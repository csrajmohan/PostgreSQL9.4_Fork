...
PlanState* check_spill_node(PlanState *node);	/* <Added on 12/05/2015 for QUEST> */
...
//	Assert(query_continue_flag==true);
	...
	modified_planstate = check_spill_node(planstate);

	if(modified_planstate == NULL)
		goto skipped_execution;
	...
	for (;;)
	{
		...
		/* <Added on 12/05/2015 for QUEST>
		 * planstate is replaced by modified_planstate
		 */
		slot = ExecProcNode(modified_planstate);
//		slot = ExecProcNode(planstate);
		...
		/* <Added on 12/05/2015 for QUEST> */
			...
			{
				printf("\tThis query has exceeded cost limit:%lf\n\n",cost_limit);
	...
	/* Estimating selectivity of spilled node */
	if(spill_node!=-1)
	{
		double spilled_node_selectivity;
		if(modified_planstate->lefttree!=NULL&&modified_planstate->righttree!=NULL)
			spilled_node_selectivity=modified_planstate->instrument->tuplecount/(modified_planstate->lefttree->plan->plan_rows * modified_planstate->righttree->plan->plan_rows);
		else
		{
			ScanState *scan_node;
			scan_node=(ScanState*)modified_planstate;
			spilled_node_selectivity=modified_planstate->instrument->tuplecount/scan_node->ss_currentRelation->rd_rel->reltuples;
		}
		/*selectivity condition check*/
		if(spilled_node_selectivity>1.0)
			spilled_node_selectivity=1.0;
		printf("\n\tSelectivity of Spilled Node=%lf\n\n",spilled_node_selectivity);
	}
...
//	query_continue_flag=true;
	spill_node=DEFAULT_SPILL_NODE;

//	printf("\n\tTotal pages read=%lf\n",total_pages_read);
//	printf("query_exec_time=%f",query_exec_time);
...
// rajmohan quest
PlanState* check_spill_node(PlanState *node)
{
	int 		l_rel;
	int 		r_rel;
	int 		temp;
	Plan 		*plan_node=node->plan;
	Plan 		*left_node;
	Plan 		*right_node;
	PlanState 	*current_planstate=node;
	...
	if(r_rel!=0)
	{
		while(true)
		{
			left_node=plan_node->lefttree;
			right_node=plan_node->righttree;
			if(left_node!=NULL&&(bms_is_member(l_rel,left_node->relids)&&bms_is_member(r_rel,left_node->relids)))
			{
				current_planstate=current_planstate->lefttree;
				plan_node=left_node;
			}
			else if(right_node!=NULL&&(bms_is_member(l_rel,right_node->relids)&&bms_is_member(r_rel,right_node->relids)))
			{
				current_planstate=current_planstate->righttree;
				plan_node=right_node;
			}
			else if(bms_is_member(l_rel,plan_node->relids)&&bms_is_member(r_rel,plan_node->relids))
			{
				break;
			}
			else
			{
				elog(ERROR,"Wrong input for spill node (relation number does not exists)");
				return(NULL);
			}
		}
	}
	else
	{
		while(true)
		{
			left_node=plan_node->lefttree;
			right_node=plan_node->righttree;
			if(left_node!=NULL&&(bms_is_member(l_rel,left_node->relids)))
			{
				current_planstate=current_planstate->lefttree;
				plan_node=left_node;
			}
			else if(right_node!=NULL&&(bms_is_member(l_rel,right_node->relids)))
			{
				current_planstate=current_planstate->righttree;
				plan_node=right_node;
			}
			else if(bms_is_member(l_rel,plan_node->relids))
			{
				IndexScanState *index;
				PlanState *left_ps;
				BitmapIndexScanState *bitmap_scan;
				switch(current_planstate->type)
				{
				case T_IndexScanState:
					index=(IndexScanState*)current_planstate;
					if(index->iss_NumRuntimeKeys != 0 && !index->iss_RuntimeKeysReady)
					{
						elog(ERROR, "This node is right subtree of nested loop join. So spilling can not be done on this node.");
						return(NULL);
					}
					break;
				case T_BitmapHeapScanState:
					left_ps=current_planstate->lefttree;
					bitmap_scan=(BitmapIndexScanState*)left_ps;
					if(bitmap_scan->biss_NumRuntimeKeys !=0 && !bitmap_scan->biss_RuntimeKeysReady)
					{
						elog(ERROR,"This node is right subtree of nested loop join. So spilling can not be done on this node.");
						return(NULL);
					}
					break;
				default:			//other cases are not needed.
					break;
				}
				break;
			}
