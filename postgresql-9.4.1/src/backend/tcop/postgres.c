...
/* Added by Rajmohan on 26/06/2014 */

double cost_limit = DEFAULT_COST_LIMIT;
...
exec_simple_query(const char *query_string)
{
	...
	foreach(parsetree_item, parsetree_list)
	{
		...
		/* <Added by Rajmohan on 28/06/2014 for QUEST> */
		query_cost = 0.0;
...
check_stack_depth(void)
{
	...
	if (stack_depth > max_stack_depth_bytes &&
		stack_base_ptr != NULL)
	{
		return;  /* <Added by Rajmohan on 28/06/2014 for QUEST> */
		ereport(ERROR,
	...
	for (;;)
	{
		...
		switch (firstchar)
		{
			case 'Q':			/* simple query */
				{
					const char *query_string;
					const char *actual_query;

					/* Set statement_timestamp() */
					SetCurrentStatementStartTimestamp();

					actual_query = pq_getmsgstring(&input_message);

					///////////////////////////// START OF FPC RELATED CODE //////////////////////////////
					char *idx;
					int pos;

					StringInfoData fpc_str, query_str,  embed_query_str;
					char *posselect = strstr(actual_query, "select ");
					unsigned char bnoncmd_select = posselect - actual_query;

					/* TODO : replace these ugly checks with good logic soon */
					if(bnoncmd_select != 0)
					{
						posselect = strstr(actual_query, "explain ");
						bnoncmd_select = posselect - actual_query;
						if(bnoncmd_select != 0)
						{
							posselect = strstr(actual_query, "SELECT ");
							bnoncmd_select = posselect - actual_query;
						}
						if(bnoncmd_select != 0)
						{
					...
					if( (bnoncmd_select == 0) && (((idx = strstr(actual_query,"fpc ")) != NULL) || ((idx = strstr(actual_query,"FPC ")) != NULL)))
					{
						/* Separate query part and fpc file from the input query and pass
						 * query part for normal processing */
					   PG_TRY();
					   {
						   initStringInfo(&fpc_str);
						   initStringInfo(&query_str);
						   initStringInfo(&embed_query_str);

						   pos = idx - actual_query;
						   strncpy(query_str.data, actual_query, pos);
						   query_str.data[pos] = '\0';

						   while (*idx != '\0' && *idx != ' ') {idx++;}

						   appendStringInfoString(&fpc_str,idx);

						   for( pos = 0; pos < strlen(fpc_str.data);pos++)
						   {
							   if(fpc_str.data[pos] == ';')
								   fpc_str.data[pos] = '\0';
						   }

						   appendStringInfoString(&embed_query_str,"/*# ");
						   appendStringInfoString(&embed_query_str,fpc_str.data);
						   appendStringInfoString(&embed_query_str,"*/");
						   appendStringInfoString(&embed_query_str,query_str.data);
						   appendStringInfoChar(&embed_query_str,';');

//						   printf("Query : %s\n",embed_query_str.data);

						   query_string = embed_query_str.data;
						   resetStringInfo(&fpc_str);
						   resetStringInfo(&query_str);
					   }
					   PG_CATCH();
					   {
						   elog(ERROR,"Could not handle FPC..Ignoring it :-( ");
						   query_string = actual_query;
					   }
					   PG_END_TRY();
					}
					else
					{
					   /* No FPC - No Extra work */
					   query_string = actual_query;
					}
					///////////////////////////// END OF FPC RELATED CODE //////////////////////////////
			...
			case 'P':			/* parse */
				{
					...
					actual_query = pq_getmsgstring(&input_message);       // FPC
