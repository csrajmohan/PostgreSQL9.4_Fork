			 ...
			 ParamListInfo params, DestReceiver *dest)
{
	...
	/* added for QUEST in July 2014 */
	/* This extra line appears at the end of explain and explain analyse outputs
	 * which conveys whether execution is completed or partially stopped
	 */

	if(es.format == EXPLAIN_FORMAT_TEXT)		/* dont want this on xml outputs..only reqd for quest */
	{
		if(query_continue_flag == true)
			do_text_output_oneline(tstate, "Completed");
		else
			do_text_output_oneline(tstate, "Not Completed");
	}
			...
			ExplainState *es)
{
		...
		planstate->instrument && planstate->instrument->nloops > 0)
	{
		...
		double		startupCost = planstate->startup_cost;
		double 		totalCost = planstate->total_cost;

		if (es->format == EXPLAIN_FORMAT_TEXT)
		{
			if(show_actual_cost)
			{
				appendStringInfo(es->str, " (actual cost=%.2f..%.2f rows=%.0f loops=%.0f)",
